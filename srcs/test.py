...
for i in range(len(x_values)):
    ...
    prediction = {
        "Slytherin": SlytherinData.predict(data_to_predict),
        "Gryffindor": GryffindorData.predict(data_to_predict),
        "Ravenclaw": RavenclawData.predict(data_to_predict),
        "Hufflepuff": HufflepuffData.predict(data_to_predict),
    }

    bad += 1 if max(prediction, key=prediction.get) != y_value[i] else 0
    print(
        f"Prediction {max(prediction, key=prediction.get)}:{y_value[i]} Real {GREEN + 'GOOD' + END if max(prediction, key=prediction.get) == y_value[i] else RED + 'BAD' + END}"
    )
